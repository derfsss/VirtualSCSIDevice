...
void Handle_CMD_Read(struct VirtIOSCSIBase *libBase, struct IOStdReq *ioreq)
{
    ...
    if (!data || length == 0) {
        DPRINTF(libBase->IExec, "[virtioscsi:cmd_read.c] CMD_READ: BADADDRESS data=%p len=%lu\n",
                (void *)data, length);
    ...
    if (length < blksz) {
        DPRINTF(libBase->IExec, "[virtioscsi:cmd_read.c] CMD_READ: BADLENGTH len=%lu < blksz=%lu\n",
                length, blksz);
